    ...
    Array.isArray(data) && data.length > 0
      ? data
          ...
          .map((item) => {
            ...
            const localFallback = fallbackCaseStudies.find((cs) => cs.slug === item.slug)
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mx-auto mt-16 grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
            {cards.map((card) => (
              <article
                key={card.slug}
                className="relative isolate flex flex-col justify-end overflow-hidden rounded-2xl bg-[#0A0B19] px-8 pt-80 pb-8 sm:pt-48 lg:pt-80"
              >
                {card.imageUrl ? (
                  <img
                    alt={card.title}
                ...
                <div className="absolute inset-0 -z-10 bg-linear-to-t from-[#0A0B19] via-[#0A0B19]/40" />
                <div className="absolute inset-0 -z-10 rounded-2xl inset-ring inset-ring-[#0A0B19]/10" />

                <div className="flex flex-wrap items-center gap-y-1 overflow-hidden text-sm/6 text-[#F7F7FC]/70">
