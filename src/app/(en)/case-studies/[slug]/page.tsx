...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: title.length > 36 ? title : `${title} | Case Study`,
    description,
    alternates: {
...
export default async function CaseStudyDetailPage({ params }: Props) {
  ...
  const { data } = await getCaseStudy(slug)
  const caseStudy = (data ?? null) as CaseStudySanityData | null
  const fallbackMarkdown = await getCaseStudyFallbackMarkdown(slug, 'en')
  const fallback = getCaseStudyBySlug(slug)
  ...
  const imageUrl = imageUrlFromCaseStudy(caseStudy) || fallback?.image || null
          ...
          {metrics.length > 0 ? (
            <div className="mt-8 flex flex-wrap gap-2">
              {metrics.map((metric) => (
                <span
                  key={metric}
                  className="rounded-full border border-gray-200 bg-[#FAFAFC] px-3 py-1 text-xs font-medium text-gray-700"
                >
                  {metric}
                </span>
              ))}
