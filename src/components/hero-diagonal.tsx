      ...
      <div className="relative min-h-[64vh] sm:min-h-[72vh] lg:min-h-[84vh] overflow-hidden">
        ...
        <div className={`mx-auto ${maxWidthClass} h-full grid grid-cols-12 gap-6 px-6 pt-32 sm:pt-36 md:pt-32`}>
            ...
            <nav className="text-xs sm:text-sm font-semibold tracking-wide whitespace-nowrap overflow-x-auto pb-2">
              {breadcrumbs.map((bc, idx) => (
                <span key={idx} className="inline">
                  ...
                  {bc.href ? (
                    <Link href={bc.href} className={`inline transition-colors ${idx === breadcrumbs.length - 1 ? "text-[#0A0B19] font-semibold" : "text-[#6B7280] hover:text-[#0A0B19]"}`}>
                      {bc.label}
                    </Link>
                  ) : (
                    <span className={`inline ${idx === breadcrumbs.length - 1 ? "text-[#0A0B19] font-semibold" : "text-[#6B7280]"}`}>{bc.label}</span>
                  )}
                </span>
              ))}
            </nav>
          ...
          <div className="col-span-12 sm:col-start-2 sm:col-end-11 row-start-2 z-10 pr-4 sm:pr-8 md:pr-12 xl:pr-16">
            <h1
              id="hero-heading"
              className="text-[32px] sm:text-[40px] md:text-[48px] lg:text-[64px] xl:text-[72px] font-light text-[#0A0B19] leading-[1.0] tracking-[-0.04em] mb-2 font-serif max-w-[22ch] md:max-w-[24ch]"
            >
              {title}
            </h1>
          </div>
