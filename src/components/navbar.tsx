...
function getSectionNavContext(pathname: string | null): SectionNavContext | null {
  ...
  // ITAD uses its own ribbon SectionNav in the client component
  if (pathname === '/services/itad-japan-apac-us' || pathname === '/ja/services/itad-japan-apac-us') {
    return null
  }

  // Ekahau uses its own ribbon SectionNav in the client component
  if (pathname === '/services/japan-ekahau-wireless-surveys' || pathname === '/ja/services/japan-ekahau-wireless-surveys') {
    return null
  }
...
export function Navbar({ banner }: { banner?: React.ReactNode }) {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (section) {
      const offset = isHome && hasSectionNav ? 124 : hasSectionNav ? 132 : 84
  ...
  useEffect(() => {
    ...
    const updateActiveSection = () => {
      const stickyOffset = isHome ? 156 : 132
      ...
      for (const item of sectionNavItems) {
        ...
        if (marker >= sectionTop) current = item.id
      }
  ...
  }, [hasSectionNav, isHome, pathname, sectionBasePath, sectionNavItems])
    ...
    <>
      <header
        className={clsx(
          ...
          scrolled
            ? isHome
              ? 'bg-[#F7F7FC]/80 shadow-sm backdrop-blur-xl'
      ...
      >
        ...
        >
          <div className="flex items-center gap-6 xl:flex-1">
            <Link href={withLocale('/')} title="Home" className="-m-1.5 p-1.5">
              ...
              <Logo className={clsx('h-9', isHome ? 'text-[#0A0B19]' : 'text-gray-950')} />
            </Link>
            {banner && (
          ...
          <div className="flex xl:hidden">
            <button
              ...
              className={clsx(
                ...
                isHome ? 'text-[#0A0B19]' : 'text-gray-700',
              )}
            >
          ...
          <PopoverGroup className="hidden xl:flex xl:gap-x-12">
            <Link
              ...
              className={clsx(
                ...
                isHome ? 'text-[#0A0B19]/70 hover:text-[#0A0B19]' : 'text-gray-900',
              )}
            >
              {isJapanese ? 'ホーム' : 'Home'}
            </Link>
            <Popover className="relative">
              ...
              >
                <div className="w-screen max-w-md flex-auto overflow-hidden rounded-2xl bg-white text-sm/6 shadow-lg ring-1 ring-gray-900/5 lg:max-w-3xl">
                  <div className="grid grid-cols-1 gap-x-6 gap-y-0 p-3 lg:grid-cols-2">
                    {services.map((item) => (
                      <div
                        key={item.name}
                        className="group relative rounded-lg px-3 py-2.5 transition-colors duration-150 hover:bg-[#FAFAFC]"
                      >
                        <Link href={item.href} className="text-sm font-semibold text-gray-900">
                          {item.name}
                          <span className="absolute inset-0" />
                        </Link>
                        <p className="mt-0.5 line-clamp-1 text-xs/5 text-gray-500">{item.description}</p>
                      </div>
                    ))}
                  </div>
                  <div className="border-t border-gray-100 bg-[#FAFAFC] px-6 py-3.5">
                    <Link
                      href={withLocale('/services')}
                      className="inline-flex items-center gap-2 text-sm font-semibold text-gray-900 transition-colors hover:text-gray-700"
                    >
          ...
          <div className="hidden xl:flex xl:flex-1 xl:items-center xl:justify-end xl:gap-5">
            <Link
              ...
              className={clsx(
                'text-sm/6 font-semibold transition-colors',
                isHome ? 'text-[#0A0B19]/65 hover:text-[#0A0B19]/80' : 'text-gray-700 hover:text-gray-900',
              )}
            >
              {isJapanese ? 'EN' : '日本語'}
            </Link>
            <span
              aria-hidden="true"
              className={clsx('h-4 w-px', isHome ? 'bg-[#0A0B19]/20' : 'bg-gray-300')}
            />
            <Link
              href={withLocale('/login')}
              className={clsx(
                'text-sm/6 font-semibold transition-colors',
                isHome ? 'text-[#0A0B19]/80 hover:text-[#0A0B19]' : 'text-gray-900 hover:text-gray-700',
              )}
            >
        ...
        {hasSectionNav && !isHome && (
          <div className="border-t border-[#0A0B19]/10 bg-[#F7F7FC]/95 backdrop-blur-sm">
            <nav aria-label="Page sections" className="overflow-x-auto px-3 py-2">
              <div className="mx-auto flex min-w-max items-center lg:justify-center">
                {sectionNavItems.map((item, index) => (
                  <a
                    ...
                    className={clsx(
                      'px-4 py-2.5 font-mono text-[11px] uppercase tracking-[0.14em] whitespace-nowrap transition-colors duration-200',
                      index < sectionNavItems.length - 1 && 'border-r border-[#0A0B19]/10',
                      activeSection === item.id
                        ? 'text-[#0A0B19]'
                        : 'text-[#0A0B19]/65 hover:text-[#0A0B19]/80',
                    )}
                  >
        ...
        <Dialog
          ...
          className="xl:hidden"
        >
          <DialogPanel className="fixed inset-0 z-50 h-[100svh] w-full overflow-y-auto bg-[#0A0B19] px-4 pt-[68px] pb-6">
            <div className="fixed inset-x-0 top-0 z-10 flex items-center justify-between bg-[#0A0B19] px-6 py-4">
              <Link
                href={withLocale('/')}
                title="Home"
      ...
      <div className={clsx(hasSectionNav && !isHome ? 'h-[124px]' : 'h-[68px]')} />
    </>
  )
}
